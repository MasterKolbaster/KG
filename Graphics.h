...
private:
	...
	// Методы для действий с фигурами
public:
	void RotateLineAroundPoint(float x1, float y1, float& x2, float& y2, float& angle);		
...
public:
	std::vector<triangle> RobertsAlgorithm(std::vector<triangle>& vecTrianglesToRaster, fPoint3D& view_point, fPoint3D& barycenter,
		int16_t sym = PIXEL_SOLID, int16_t col = FG_DARK_YELLOW, int16_t col_edge = FG_GREY);
